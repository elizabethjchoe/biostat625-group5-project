...
```{r}
## need to re-label the transformed data! for unity 
data <- read.csv("./liz/transformed_data.csv")
...
Closer look at the socioeconomic variables: insurance (PRIMINSR), doctor (PERSDOC3), costs (MEDCOST1), checkups (CHECKUP1), education (EDUCA), employment status (EMPLOY1), income (INCOME3)
```{r}
# heart disease
# renamed <- read.csv("liz/subsetted_cols_RENAME.csv")
# heartdisease <- renamed[,"HEARTDISEASE"]

# SES variables: 
ses_subset <- data[,c("PRIMINSR", "PERSDOC3", "MEDCOST1", "CHECKUP1", "EDUCA", "EMPLOY1", "INCOME3")]

table(ses_subset$PRIMINSR)
```
